...
void Player_AI::engage(){
    ...
    if (m_follow && chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - m_elapsed_engage) < m_engagementRate) {skipSearch = true;}
    if (!skipSearch){
...
void Player_AI::moveToTarget(){
    ...
    if (abs(diffX) < m_speed || abs(diffY) < m_speed) {return;}
    double desiredAngle = asin(abs(diffX) / (sqrt((diffX)*(diffX) + (diffY)*(diffY)))) * 180.0 / PI;
    if (diffX > 0 && diffY > 0) {desiredAngle = 180 - desiredAngle;}
    if (diffX < 0 && diffY > 0) {desiredAngle = desiredAngle - 180;}
    if (diffX < 0 && diffY < 0) {desiredAngle = - desiredAngle;}
    if (desiredAngle < 0) {desiredAngle += 360;}
    if (desiredAngle > m_rotationAngle + 360 && desiredAngle - m_rotationAngle - 360 > 180) {m_clockwise = -1;}
    else if (desiredAngle < m_rotationAngle + 360 && m_rotationAngle + 360 - desiredAngle < 180) {m_clockwise = -1;}
    if (m_rotationAngle < desiredAngle) {m_rotationAngle += m_clockwise * 5;}
    if (m_rotationAngle > desiredAngle) {m_rotationAngle -= m_clockwise * 5;}
    if (m_rotationAngle > 360) {m_rotationAngle -= 360;} else if (m_rotationAngle < -360) {m_rotationAngle += 360;}
    if (sqrt(diffX*diffX + diffY * diffY) <= MIN_DIST) {return;}
